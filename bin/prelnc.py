...
def main():
    ...
    for opt, arg in opts:
        if opt == '-h':
            print("prelnc.py -l/--lncFASTA <FASTA> -m/--model<pkl> -r/--hexamerfile <tsv> -o/--ofile <outputfile>")
            print("Example: prelnc.py -i transcripts.fa -m human.pkl -r human_hexamer.csv -o prediction.txt")
            sys.exit()
        elif opt in ("-i", "--ifile"):
            inputfile = arg
        elif opt in ("-m", "--model"):
...
################################### main body #####################################
    ...
    predict_result= load_model.predict_proba(data)[:,1]
